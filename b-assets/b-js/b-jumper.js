// I can only apolagise for what you are about to witness. Many attempts where made to place this code across multiple files and many man hours were lost in the process.

// The game has a few bug but nothing that will destroy it's playability
...
class Enemy {
    constructor(x, y, radius, context) {
        ...
        this.destroyed = false;
        this.deathTimer = 0; // new: countdown before removal
    }
    ...
    setState(newState) {
        ...
        switch (newState) {
            ...
            case EnemyState.DEAD:
                this.deathTimer = 60; // frames to freeze before removal
        ...
        switch (this.state) {
            ...
            case EnemyState.DEAD:
                if (this.deathTimer > 0) {
                    this.deathTimer--;
                    this.y += 1; // slowly fall
                    this.opacity = this.deathTimer / 30; // fade out
                } else {
                    this.destroyed = true;
                }
                break;
...
class PowerUp {
    constructor(x, y, type, context, duration = 10000) {
        this.x = x;
        this.y = y;
        this.width = 20; // doubled
        this.height = 20; // doubled
        this.type = type;
        this.context = context;
        this.active = true;
    ...
    draw() {
        ...
        switch (this.type) {
            ...
            case "rapidfire":
                ctx.fillStyle = "cyan";
                const bulletSpacing = w * 0.25;
                const bulletRadius = w * 0.08;
                const centerY = y + h / 2;

                for (let i = 0; i < 3; i++) {
                    const cx = x + w * 0.2 + i * bulletSpacing;
                    ctx.beginPath();
                    ctx.arc(cx, centerY, bulletRadius, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.lineWidth = 2;
                    ctx.strokeStyle = "black";
                    ctx.stroke();
        ...
        ctx.restore(); // remove glow effect for other drawings
    }
    ...
    apply(player) {
        ...
        switch (this.type) {
            case "speed":
                if (player.hasSpeedPU) return; // already active, ignore
                player.hasSpeedPU = true;
                this.active = false;
                ...
                setTimeout(() => {
                    player.playerBaseSpeed /= 2;
                    player.playerRunSpeed /= 2;
                    player.hasSpeedPU = false; // reset
                }, this.duration);
                break;

            case "jump":
                if (player.hasJumpPU) return;
                player.hasJumpPU = true;
                this.active = false;
                player.jumpSpeed *= 1.5;
                setTimeout(() => {
                    player.jumpSpeed /= 1.5;
                    player.hasJumpPU = false;
                }, this.duration);
                break;
            ...
            case "invincible":
                if (player.hasInvinciblePU) return;
                player.hasInvinciblePU = true;
                this.active = false;
                player.invincible = true;
                player.lastHitTime = Date.now();
                setTimeout(() => {
                    player.invincible = false;
                    player.hasInvinciblePU = false;
                }, this.duration);
                break;

            case "rapidfire":
                if (player.hasRapidFirePU) return; // already active
                player.hasRapidFirePU = true;
                this.active = false;
...
class Player {
    constructor(playerX, playerY, playerWidth, playerHeight, context) {
        ...
        this.hasSpeedPU = false;
        this.hasJumpPU = false;
        this.hasInvinciblePU = false;
        this.hasRapidFirePU = false;

        this.lastShotTime = 0; // timestamp of the last bullet fired
        this.shootCooldown = 300; // milliseconds between shots (adjust to taste)
    }
...
class Game {
    ...
    fireBullet(targetX, targetY) {
        if (!this.player.canShoot()) return; // too soon, ignore
        ...
        this.player.recordShot(); // update lastShotTime
    }
    ...
    dropPowerUp(x, y) {
        const types = ["speed", "jump", "health", "invincible", "rapidfire"]; // added invincibility
        const baseChance = 0.15;
        const waveBonus = 0.02 * this.waveNumber; // +2% per wave
        const chance = Math.min(0.3, baseChance + waveBonus); // cap at 30%
    ...
    updateScore() {
        ...
        // Format like a clock: MM:SS
        const minutes = Math.floor(totalSeconds / 60);
        const seconds = totalSeconds % 60;

        this.timeScore = `${minutes.toString().padStart(2, "0")}:${seconds
            .toString()
            .padStart(2, "0")}`;
    }
    ...
    checkGameOver() {
        if (!isAlive && gameState === GameState.PLAYING) {
            ...
            const finalScore = this.killScore * 50 + timeInSeconds * 5;
            ...
            this.stopGameLoop();
            setTimeout(() => {
                this.drawGameOver(finalScore, finalTime);
            }, 50);
        }
    }

    drawGameOver(finalScore, finalTime) {
